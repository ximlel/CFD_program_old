...
for j=(center-A+1):(center+A)
    ...
    elseif(floor(y_1)==floor(y_2))
       CC(j,i+l*round(column/K))=(y_1+y_2-2*floor(y_1))/2;
    else
       CC(floor(max(y_1,y_2))+center+1,i+l*round(column/K))=0;
       CC(floor(min(y_1,y_2))+center+1,i+l*round(column/K))=1;
    end
end
end
end
