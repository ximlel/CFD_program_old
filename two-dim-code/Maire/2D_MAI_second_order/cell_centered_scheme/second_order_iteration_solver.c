 ...
 double * NORMAL_VELOCITY[], double * gamma, double * cpu_time, char * limiter, double CFL)
{
  ...
  double U_p_x_record[NUM_POINT];
  double U_p_y_record[NUM_POINT];
  int RECORD;
...
LOOP:
	...
	for(k = 0; k < NUM_POINT; ++k)
				{
					 U_p_x_record[k] = U_p_x[k];
					 U_p_y_record[k] = U_p_y[k];
				}				
		  ...
		  for(j = 0; j < CELL_POINT[k][0]; ++j)
			  {
				  ...
				  EUCCLHYD_iteration(M_pc, gamma[k], P[i][k], RHO[i][k], posi_l_pc[k][j], posi_n_pc_x[k][j], posi_n_pc_y[k][j], nega_l_pc[k][j], nega_n_pc_x[k][j], nega_n_pc_y[k][j], U[i][k], V[i][k], U_p_x[CELL_POINT[k][j+1]], U_p_y[CELL_POINT[k][j+1]]);
                                 M_pc_00[k][j] = M_pc[0][0];
                                 M_pc_01[k][j] = M_pc[0][1];
                                 M_pc_10[k][j] = M_pc[1][0];
                                 M_pc_11[k][j] = M_pc[1][1];
	...
	for(k = 0; k < NUM_POINT; ++k)
				{
					 if(RECORD<50&&(fabs(U_p_x_record[k]-U_p_x[k])>eps&&fabs(U_p_y_record[k]-U_p_y[k])>eps))
						goto LOOP; 			 		 
				}
				if(RECORD>=50)
					printf("Iteration fail(>50) at step %d.\n",i);
					...
					for(j = 0; j < CELL_POINT[k][0]; ++j)
						{
							...
							EUCCLHYD_dt_iteration(M_pc, gamma[k], P[i][k], RHO[i][k], posi_l_pc[k][j], posi_n_pc_x[k][j], posi_n_pc_y[k][j], nega_l_pc[k][j], nega_n_pc_x[k][j], nega_n_pc_y[k][j], dt_posi_l_n_pc_x[k][j], dt_posi_l_n_pc_y[k][j], dt_posi_n_pc_x[k][j], dt_posi_n_pc_y[k][j], dt_nega_l_n_pc_x[k][j], dt_nega_l_n_pc_y[k][j], dt_nega_n_pc_x[k][j], dt_nega_n_pc_y[k][j]);
